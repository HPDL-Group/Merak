...
def configure_zero_optimizer(
    optimizer: optim.Optimizer, args, dynamic_loss_scale_args=None
) -> optim.Optimizer:
    ...
    optimizer = ZeroOptimizer_Stage1(
        args,
        optimizer,
        dynamic_loss_scale=args.fp16 or args.bf16,
...
class ZeroOptimizer_Stage1(object):
    ...
    def __init__(
        ...
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            self.logger.debug(f'element_intervals:{element_intervals}')
        ...
        self.overflow_checker = CheckOverflow(
            self.fp16_groups, zero_reduce_scatter=True
        )

    @staticmethod
    def best_max_elems_per_comm(num_elements, max_elements_per_comm, dp, logger):
    ...
    def get_data_parallel_sub_partitions(
        ...
        print(f'total_num_elements:{total_num_elements}, max_elements_per_comm:{max_elements_per_comm}')
    ...
    def flatten_dense_tensors_sub_partition_aligned(
        ...
        self.logger.info(
            ...
            f"sub_partition_size: {sub_partition_size}, padding: {padding}",
            ranks=[0],
        )
        self.logger.info(
            f"number of elements with padding: {num_elements} "
            f"+ {padding} = {num_elements + padding}",
            ranks=[0],
        )
    ...
    def step(self, closure=None):
        ...
        if self.overflow:
            ...
            if self.verbose and dist.get_rank() == 0:
                self.logger.debug(
                    f"[merak] fp16 dynamic loss scale overflow! Skipping step. "
        ...
        for i, group in enumerate(self.fp16_groups):
            ...
            norm_groups.append(
                get_grad_norm(local_grad_sub_partitions, partition_params, mpu=self.mpu)
            )
    ...
    def _rigid_state_dict(self):
        ...
        # Add key values for AdaCheck
        if self.args.ada_checkpoint:
            if self.global_step in self.args.check_steps:
                partition_id = dist.get_rank(group=self.dp_process_group)
                params_in_partition = []
                first_offset = []
                for i in range(len(self.params_in_rank_sub_partitions)):
                    params_for_cur_group = []
                    for comm_id in range(len(self.params_in_rank_sub_partitions[i][partition_id])):
                        if comm_id == 0:
                            offset_for_cur_partition = self.params_in_rank_sub_partitions_offsets[i][partition_id][comm_id][0]
                        for j in range(len(self.params_in_rank_sub_partitions[i][partition_id][comm_id])):
                            if self.params_in_rank_sub_partitions_offsets[i][partition_id][comm_id][j] == 0 or comm_id == 0:
                                params_for_cur_group.append(self.params_in_rank_sub_partitions[i][partition_id][comm_id][j])

                    first_offset.append(offset_for_cur_partition)
                    params_in_partition.append(params_for_cur_group)

                state_dict['params_in_partition'] = params_in_partition
                state_dict['first_offset'] = first_offset
                state_dict['partition_size'] = self.sub_partition_sizes 
    ...
    def load_state_dict(
        self, state_dict, load_optimizer_states=True, load_from_fp32_weights=False
    ):
        """
        ...
        """
        self._rigid_load_state_dict(state_dict, load_optimizer_states)
