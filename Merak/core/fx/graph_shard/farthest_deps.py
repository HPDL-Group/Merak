...
from .utils import _create_shard_to_param_count, _snake_case
...
def farthest_deps_mapping(
    traced_graph_module: GraphModule, shard_count: int, node_user_count=None
    ...
    logger.info(f"Total number of params are {param_count['']}", ranks=[0])
    per_shard_param = (param_count[""] - excluded_count) // shard_count
    logger.info(f"Per shard param count {per_shard_param}", ranks=[0])
    logger.info(f"Node count {len(traced_graph_module.graph.nodes)}", ranks=[0])
        ...
        if node.op == "placeholder":
            ...
            for arg in node.args:
                ...
                if arg.name in node_name_to_shard_id and arg.name != nodes_so_far[-1]:
                    if (
                        node_name_to_shard_id[arg.name] < min_shard_id
                        and arg.name not in node_user_count
                    ):
            ...
            if (
                ...
                # 则需要成为这个shard的output
                extra_output[shard_id] = [
                    k
                    for k in node_user_count
...
def farthest_deps_split(
    ...
    node_name_to_shard_id, shard_output, func_inputs, extra_output = (
        farthest_deps_mapping(traced_graph_module, args.shard_count, output_nodes_count)
    )

    # split graph
    module_list, func_inputs, min_deps_partitions = split_module(
