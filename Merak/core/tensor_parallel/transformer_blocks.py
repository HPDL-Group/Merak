...
class PipedGPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        self.overlap_level = config.tp_overlapping_level
    ...
    def forward(
        ...
        if self.overlap_level == 2:
            hidden_states_chunks = PipedTPFunction.apply(
                self, attention_mask_chunks, head_mask, *hidden_states_chunks
            )
        else:
            hidden_states_chunks = CrossLayerPipedTPFunction.apply(
                self, attention_mask_chunks, head_mask, *hidden_states_chunks
            )
...
class PipedGPT2Model(GPT2Model):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList(
            [
                PipedGPT2Block(config, layer_idx=i)
...
class PipedTPFunction(torch.autograd.Function):
    ...
    def forward(ctx, run_functions, attention_mask, head_mask, input_1, input_2):
        ...
        with torch.enable_grad():
            ...
            attention_output_1, attention_output_1_bias = run_functions.self_attention(
                ln1_out, attention_mask=attention_mask[0], head_mask=head_mask
            )
            ...
            attention_output_2, attention_output_2_bias = run_functions.self_attention(
                ln2_out, attention_mask=attention_mask[1], head_mask=head_mask
            )

            ## fp 3
            sync_comm()
            layernorm_input_1 = run_functions.drop_residual(
                attention_output_1 + attention_output_1_bias, input_1_clone
            )
            ...
            ## fp 4
            sync_comm()
            layernorm_input_2 = run_functions.drop_residual(
                attention_output_2 + attention_output_2_bias, input_2_clone
            )
    ...
    def backward(ctx, *grads):
        ...
        torch.autograd.backward(
            (layernorm_input_2, post_attn_ln_2), (post_ln_2_in.grad, post_ln_2_out.grad)
        )

        ## bp4
        sync_comm()
        torch.autograd.backward(
            (layernorm_input_1, post_attn_ln_1), (post_ln_1_in.grad, post_ln_1_out.grad)
        )
...
class CrossLayerPipedTPFunction(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, run_functions, attention_mask, head_mask, input_1, input_2, *inputs
    ):
        ...
        with torch.enable_grad():
            ...
            if run_functions.is_first_layer:
                ...
                input_1 = run_functions.drop_residual(
                ...
                input_2 = run_functions.drop_residual(
        ...
        if run_functions.is_last_layer:
            ctx.save_for_backward(
                mlp_output_2_detached,
                post_ln_2_in_detached,
                mlp_output_1_detached,
    ...
    def backward(ctx, *grads):
        ...
        if is_last_layer:
            ...
            sync_comm()
            torch.autograd.backward(
                (layernorm_input_1, post_attn_ln_1),
        ...
        if is_first_layer:
            ...
            return (
                ...
                None,
                None,
                mlp_output_1_detached.grad,
                mlp_output_2_detached.grad,
                post_ln_1_in_detached.grad,
                post_ln_2_in_detached.grad,
            )
