    ...
    def forward(
        ...
        if self.reorder_and_upcast_attn:
            attn_output, attn_weights = \
                self._upcast_and_reordered_attn(query, key,
...
class PipedGPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        assert not config.add_cross_attention, \
            'only support self attention for now'
        ...
        inner_dim = config.n_inner \
            if config.n_inner is not None else 4 * hidden_size
    ...
    def forward(
        ...
        # 'only support data with attention_mask'
        ...
        if self.overlap_level == 2:
            hidden_states_chunks = \
                PipedTPFunction.apply(self, attention_mask_chunks,
                                      head_mask, *hidden_states_chunks)
        else:
            hidden_states_chunks = \
                CrossLayerPipedTPFunction.apply(self, attention_mask_chunks,
                                                head_mask,
                                                *hidden_states_chunks)
...
class PipedGPT2Model(GPT2Model):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList([PipedGPT2Block(config, layer_idx=i)
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            # this attention 
            # mask is more simple than the triangular masking of causal 
            # attention used in OpenAI GPT, we just need to prepare the 
...
class PipedTPFunction(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, run_functions, attention_mask,
                head_mask, input_1, input_2):
        ...
        with torch.enable_grad():
            ...
            sync_comm()
            layernorm_input_1 = \
                run_functions.drop_residual(
                                    attention_output_1+attention_output_1_bias,
                                    input_1_clone)
            post_attn_ln_1 = \
                run_functions.post_attention_layernorm(layernorm_input_1)
            ...
            layernorm_input_2 = \
                run_functions.drop_residual(
                                    ...
                                    input_2_clone)
            post_attn_ln_2 = \
                run_functions.post_attention_layernorm(layernorm_input_2)
            ...
            ## fp 5
            sync_comm()
            output_1 = run_functions.drop_residual(
                                    mlp_output_1+mlp_bias_1, post_ln_1_in)


            ## fp 6
            sync_comm()
            output_2 = run_functions.drop_residual(
                                    mlp_output_2+mlp_bias_2, post_ln_2_in)



        ctx.save_for_backward(input_1_clone, input_2_clone,
                              layernorm_output_1, layernorm_output_2,
                              ln1_out, ln2_out, layernorm_input_1,
                              post_attn_ln_1,
    ...
    def backward(ctx, *grads):
        ...
        layernorm_input_2, post_attn_ln_2, post_ln_1_in, post_ln_1_out, \
        output_1, post_ln_2_in, post_ln_2_out, output_2 = ctx.saved_tensors
        ...
        ## bp3
        sync_comm()
        torch.autograd.backward((layernorm_input_2, post_attn_ln_2),
                                (post_ln_2_in.grad, post_ln_2_out.grad))

        ## bp4
        sync_comm()
        torch.autograd.backward((layernorm_input_1, post_attn_ln_1),
                                (post_ln_1_in.grad, post_ln_1_out.grad))
...
class CrossLayerPipedTPFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, run_functions, attention_mask,
                head_mask, input_1, input_2, *inputs):
        ctx.layer_states = (run_functions.is_first_layer,
                            run_functions.is_last_layer)
        ...
        with torch.enable_grad():
            ...
            if run_functions.is_first_layer:
                ...
                layernorm_output_1 = \
                    run_functions.input_layernorm(input_1_clone)
                ...
                layernorm_output_2 = \
                    run_functions.input_layernorm(input_2_clone)
                ...
                input_1 = run_functions.drop_residual(mlp_output_1_detached,
                                                      post_ln_1_in_detached)
                input_1_clone = input_1.detach()
                input_1_clone.requires_grad = True
                ...
                input_2 = \
                    run_functions.drop_residual(mlp_output_2_detached,
                                                post_ln_2_in_detached)
            ...
            if run_functions.is_last_layer:
                ...
                sync_comm()
                output_1 = \
                    run_functions.drop_residual(mlp_output_1, post_ln_1_in)


                ## fp 6, only last layer
                sync_comm()
                output_2 = \
                    run_functions.drop_residual(mlp_output_2, post_ln_2_in)
        ...
        if run_functions.is_last_layer:
            ...
            return mlp_output_1.detach(), mlp_output_2.detach(), \
                   post_ln_1_in.detach(), post_ln_2_in.detach()
    ...
    def backward(ctx, *grads):
        ...
        if is_last_layer:
            ...
            mlp_output_1_grad = grads[0]
            mlp_output_2_grad = grads[1]
            post_ln_1_in_grad = grads[2]
            post_ln_2_in_grad = grads[3]
            ...
            sync_comm()
            torch.autograd.backward((layernorm_input_2, post_attn_ln_2),
                                    (post_ln_2_in_grad, post_ln_2_out.grad))


            ## bp4
            sync_comm()
            torch.autograd.backward((layernorm_input_1, post_attn_ln_1),
                                    (post_ln_1_in_grad, post_ln_1_out.grad))
