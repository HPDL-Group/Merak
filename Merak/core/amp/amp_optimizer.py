...
# https://github.com/microsoft/DeepSpeed/blob/v0.5.10/deepspeed/runtime/fp16/unfused_optimizer.py

import torch
try:
    from torch.distributed.tensor import DTensor, zeros
except ImportError:
    from torch.distributed._tensor import DTensor, zeros
...
class HalfPrecisionOptimizer:
    ...
    def __init__(
        ...
        self.overflow = False
        self.overflow_checker = CheckOverflow(self.fp16_groups, mpu=mpu)
    ...
    def step(self):
        ...
        if self.overflow:
            logger.debug(
                f"[Merak] fp16 dynamic loss scale overflow! Skipping step {self.cur_iter}."
                f"Attempted loss scale: {prev_scale}, reducing to {self.cur_scale}",
            )
        ...
        for fp32_group, fp16_group in zip(self.fp32_groups, self.fp16_groups):
            for _, (fp32_param, fp16_param) in enumerate(zip(fp32_group, fp16_group)):
    ...
    def state_dict(self):
        """
        Returns a dict containing the current state of this :class:`HalfPrecisionOptimizer`
        instance. This dict contains attributes of :class:`HalfPrecisionOptimizer`,
        ...
        if self.args.ada_checkpoint:
            if self.global_step in self.args.check_steps:
                copy_fp16_groups = []
                for params in self.fp16_groups:
                    copy_fp16_group = []
                    for param in params:
                        detach_param = param.detach()
                        # p_name = nvmapping.get_name_from_parameters(param)
                        # nvmapping.optimizer_param_name_mapping(detach_param, p_name)
                        copy_fp16_group.append(detach_param)
                    copy_fp16_groups.append(copy_fp16_group)
                state_dict["fp16_groups"] = copy_fp16_groups
        return state_dict
    ...
    def load_state_dict(self, state_dict, load_optimizer_states=True):
        ...
        Example::
            ...
            optimizer = HalfPrecisionOptimizer(optimizer, static_loss_scale = 128.0)
            ...
            checkpoint = torch.load("saved.pth")
            model.load_state_dict(checkpoint['model'])
            optimizer.load_state_dict(checkpoint['optimizer'])
        """
        # I think it should actually be ok to reload the optimizer before the
        # model.
