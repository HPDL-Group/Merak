...
class BaseTunerLayer(ABC):
    ...
    def disable_adapters(self) -> bool:
        # use a property to ensure that disable_adapters is not set directly, instead use the
        # enable_adapters method
        return self._disable_adapters

    @property
    def active_adapter(self) -> str:
        # use a property to ensure that active_adapter is not set directly, instead use the
        # set_adapter method
        ...
        for layer_name in self.adapter_layer_names:
            ...
            for key, layer in module_dict.items():
                if key in adapter_names:
                    # Note: It is possible that not a single layer is called with requires_grad_
                    # (True) here. This may happen if a completely different adapter layer is being
                    # activated.
...
class LoraLayer(BaseTunerLayer):
    ...
    def _init_empty_weights(self, cls, *args, **kwargs) -> None:
        # A helper method that allows to initialize the layer of the given class without spending
        # time to initialize the model weights. The implementation is inspired by
        # https://pytorch.org/docs/stable/generated/torch.nn.utils.skip_init.html but this function
        # cannot be used directly.
        # Instead of this approach, it would be possible to bypass the __init__ of the class but
        # that runs the risk of omitting important logic inside that __init__.
        kwargs = kwargs.copy()
...
class Linear(nn.Linear, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Note that we don't use self._init_empty_weights() for Linear because it is a bit slower
        # and the benefit of added robustness is not big enough for Linear.
    ...
    def merge(self, safe_merge: bool = False) -> None:
        ...
        Args:
            safe_merge (`bool`, *optional*):
                If True, the merge operation will be performed in a copy of the original weights
                and check for NaNs before merging the weights. This is useful if you want to check
                if the merge operation will produce NaNs. Defaults to `False`.
        """
        if self.merged:
            warnings.warn(
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        # float16 while being on CPU, we need to cast the weights to float32, perform the merge and
        # then cast back to float16 because the `@` and matmul operation in general is not
...
class Embedding(nn.Embedding, LoraLayer):
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        # In case users wants to merge the adapter weights that are in float16 while being on CPU,
        # we need to cast the weights to float32, perform the merge and then cast back to float16
        # because the `@` and matmul operation in general is not supported in torch + cpu + fp16.
        cast_to_fp32 = device.type == "cpu" and dtype == torch.float16
