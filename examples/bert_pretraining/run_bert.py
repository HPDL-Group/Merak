...
import torch
from bert_data import HDF5Dataset, WorkerInitObj
from config import load_config
from lamb import Lamb
from schedulers import PolyWarmUpScheduler
from transformers import BertConfig, BertForPreTraining, HfArgumentParser, set_seed
...
def parse_option(parser):
    ...
    parser.add_argument(
        "--dataset-name", type=str, help="name of dataset from the datasets package"
    )
    ...
    parser.add_argument(
        "--max-pred-length", type=int, default=20, help="max sequence length"
    )
    parser.add_argument(
        "--warmup-proportion",
        type=float,
...
def main():
    ...
    train_dataset = HDF5Dataset(
        args, training_args, json_file=json_file, max_pred_length=pred_length
    )
    ...
    class BertTrainer(MerakTrainer):
        ...
        def load_checkpoint(self, resume_from_checkpoint):
            if os.path.exists(resume_from_checkpoint):
                iteration, state_dict, _ = self._load_checkpoint(
                ...
                for iter, item in enumerate(state_dict["optimizer"]["param_groups"]):
                    ...
                    state_dict["optimizer"]["param_groups"][iter][
                        "t_total"
                    ] = self.args.max_steps
                    state_dict["optimizer"]["param_groups"][iter][
                        "warmup"
                ...
                if "master params" in state_dict.keys():
                    for param, saved_param in zip(
                        amp.master_params(self.train_engine.optimizer),
