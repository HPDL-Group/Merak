...
from transformers import HfArgumentParser, ViTConfig, ViTForImageClassification, set_seed
    ...
    class ViTTrainer(MerakTrainer):
        ...
        def evaluation(self):
            ...
            if self.summary_writer is not None:
                self.summary_events = [
                    (f"Eval/loss", loss.mean().item(), self.eval_params.global_steps),
                ]
                for event in self.summary_events:  # write_summary_events
                    self.summary_writer.add_scalar(event[0], event[1], event[2])
                avg_acc = list(metrics.avg.values())[0]
                self.summary_events = [
                    (f"Eval/Accuracy", avg_acc, self.eval_params.global_steps),
                ]
                for event in self.summary_events:  # write_summary_events
                    self.summary_writer.add_scalar(event[0], event[1], event[2])
